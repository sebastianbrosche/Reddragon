...
class TalkingNPC(Object):
    """
    An NPC capable of holding a menu-driven conversation.
    Uses Evennia's TalkingNPC contrib as inspiration.
    """
    
    def at_object_creation(self):
        super().at_object_creation()
        self.db.is_npc = True
        self.db.is_mob = False  # Not a combat mob
        self.db.dialogue = {}  # {node_name: {text, options}}
        
    def at_talk(self, character):
        """Called when a character talks to this NPC."""
        from evennia.utils.evmenu import EvMenu
        
        # Default simple menu
        def menu_start(caller):
            text = f"'{self.db.desc or f'Hello, I am {self.key}.'}'"
            options = [
                {"desc": "Goodbye", "goto": "END"},
            ]
            return text, options
        
        def END(caller):
            text = f"'{self.key} nods.'"
            return text, None
        
        EvMenu(character, {"menu_start": menu_start, "END": END}, 
               startnode="menu_start", cmd_on_quit="look")
