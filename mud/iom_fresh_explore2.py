#!/usr/bin/env python3
"""
Islands of Myth - Fresh Character + World Explorer v2
Correctly handles the 'c' create prompt.
"""
...
OUTPUT_FILE = "/root/.openclaw/workspace/mud/iom_fresh_explore2.txt"

with open(OUTPUT_FILE, 'w') as f:
    f.write("# Islands of Myth — Fresh Character + World Exploration v2\n# " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n\n")
...
# Send name at login menu
tn.write(CHAR_NAME)
time.sleep(1)
text = read_all(tn, 1)
write_out("NAME ENTRY", text)

# If prompted to create, send 'c'
...
# Now should ask for name again or confirm
if "correct" in text.lower() or "is that correct" in text.lower():
    tn.write(b'y\n')
    time.sleep(1)
    text = read_all(tn, 1)
    write_out("NAME CONFIRM", text)
elif "please enter" in text.lower() and "name" in text.lower():
    # Asked for name again
    tn.write(CHAR_NAME)
    time.sleep(1)
    text = read_all(tn, 1)
    write_out("NAME RE-ENTRY", text)
    if "correct" in text.lower():
        tn.write(b'y\n')
        time.sleep(1)
        text = read_all(tn, 1)
        write_out("NAME CONFIRM 2", text)
