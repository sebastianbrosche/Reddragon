#!/usr/bin/env python3
"""
Islands of Myth - Enter the World v2
Quits pager before sending enter portal.
"""
...
OUTPUT_FILE = "/root/.openclaw/workspace/mud/iom_enter_world2.txt"

with open(OUTPUT_FILE, 'w') as f:
    f.write("# Islands of Myth — Enter the World & Explore v2\n# " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n\n")
...
# Read sign - might trigger pager, handle it
tn.write(b'look sign\n')
time.sleep(1)
text = ""
for _ in range(20):  # Read through pager
    chunk = tn.read_very_eager().decode('ascii', errors='replace')
    text += chunk
    if "--More--" in chunk:
...
# Make sure we're back at prompt, then enter portal
time.sleep(1)
tn.read_very_eager()
...
# Explore in each direction from first real room
