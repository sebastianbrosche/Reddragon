#!/usr/bin/env python3
"""
Fresh character creator + single guild visitor.
Creates a new char, skips tutorial, picks human, enters world,
navigates to guild, visits target guild, captures data, quits.
"""
...
GUILD_NAME = sys.argv[1] if len(sys.argv) > 1 else "elemental"
CHAR_NAME = f"scout{random.randint(1000,9999)}"
...
def read_until_prompt(tn, wait=2):
    ...
    for _ in range(15):
        try:
            ...
            break
        all_text += chunk
        if "hp(" in chunk or "Your Selection?" in chunk or "Please type" in chunk or "Would you like" in chunk:
            break
        if chunk:
            time.sleep(0.3)
        else:
...
# Step 6: Referral (blank)
tn.write(b'\n')
text = read_until_prompt(tn, 2)
write_out("REFERRAL", text)

# Step 7: Skip tutorial
tn.write(b'yes\n')
...
# From starting room (Adventurer Guild Entrance), just go southwest to Portal Room
# But first verify
tn.write(b'look\n')
text = read_all(tn, 1)
write_out("LOCATION", text)

# If at guild entrance, go to Portal Room
if "Adventurer" in text or "Guild" in text:
    print("[*] At guild entrance.", file=sys.stderr)
else:
    # Try to find guild entrance
    print("[*] Not at guild, trying north...", file=sys.stderr)
    tn.write(b'north\n')
    text = read_all(tn, 1)
