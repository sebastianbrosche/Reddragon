#!/usr/bin/env python3
"""
Islands of Myth - Tutorial Walker v3
Actually handles compound commands like 'look at sign'.
"""
...
OUTPUT_FILE = "/root/.openclaw/workspace/mud/iom_tutorial_walk3.txt"

with open(OUTPUT_FILE, 'w') as f:
    f.write("# Islands of Myth — Tutorial Walk-Through v3\n# " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n\n")
...
while step < 200:
    ...
    if "type 'look at sign'" in lower or "look at sign" in lower:
        ...
        # Check if we're in a real game room (not tutorial room)
        if "guide" not in lower and "tour" not in lower and "void of white" not in lower:
            ...
            # Still in tutorial, send enter to advance
            print(f"[*] Step {step}: Tutorial prompt, sending enter", file=sys.stderr)
            tn.write(b'\n')
    else:
        # No specific prompt detected, send enter
        if text.strip():
