...
class Exit(DefaultExit):
    ...
    def at_object_creation(self):
        ...
        # Add shorthand aliases for movement directions
        shorthand_map = {
        ...
        if key_lower in shorthand_map:
            for alias in shorthand_map[key_lower]:
    ...
    def at_init(self):
        """
        Called when object is reloaded from cache.
        Rebuild the exit cmdset after parent removes old one.
        """
        super().at_init()
        # Rebuild exit cmdset after reload (at_init removes old one)
        if not self.cmdset.has_cmdset("ExitCmdSet", must_be_default=True):
