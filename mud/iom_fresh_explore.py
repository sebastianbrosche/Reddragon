#!/usr/bin/env python3
"""
Islands of Myth - Fresh Character + World Explorer
Creates a brand new character, goes through creation, enters world, explores.
"""
...
OUTPUT_FILE = "/root/.openclaw/workspace/mud/iom_fresh_explore.txt"

with open(OUTPUT_FILE, 'w') as f:
    f.write("# Islands of Myth — Fresh Character + World Exploration\n# " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n\n")
...
# Check if name exists or is new
if "correct" in text.lower() or "is that correct" in text.lower():
    tn.write(b'y\n')
    time.sleep(1)
    text = read_all(tn, 1)
    write_out("NAME CONFIRM", text)
elif "already exists" in text.lower():
    print("[*] Name exists, trying password...", file=sys.stderr)
...
# Now we should be in the real world with a race selected
...
# Try movement in all directions from starting room
