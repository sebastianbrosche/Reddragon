#!/usr/bin/env python3
"""
Fresh character creator + single guild visitor (READ SIGN REQUIRED).
New characters must read the sign before entering the portal.
"""
...
def read_until_prompt(tn, wait=2):
    ...
    for _ in range(15):
        try:
            ...
            break
        all_text += chunk
        if "hp(" in chunk or "Your Selection?" in chunk or "New character name:" in chunk or "Please enter" in chunk or "Would you like" in chunk or "correct?" in chunk.lower() or "password:" in chunk.lower():
            break
        if chunk:
            time.sleep(0.3)
        else:
...
# Step 3: Send name at "New character name:"
tn.write(f"{CHAR_NAME}\n".encode())
text = read_until_prompt(tn, 2)
write_out("NAME ENTRY 2", text)

# Step 4: Confirm name if prompted
...
# Step 8: Referral (blank)
tn.write(b'\n')
text = read_until_prompt(tn, 2)
write_out("REFERRAL", text)

# Step 9: Skip tutorial
tn.write(b'yes\n')
...
# Step 11: READ THE SIGN (required for new characters)
print("[*] Reading sign...", file=sys.stderr)
tn.write(b'read sign\n')
text = read_all(tn, 2, max_chunks=30)
write_out("READ SIGN", text)

# If FAQ menu appeared, exit it
if "continue" in text.lower() or "out" in text.lower() or "back" in text.lower():
    print("[*] FAQ menu detected, sending 'out'...", file=sys.stderr)
    tn.write(b'out\n')
    text = read_all(tn, 2, max_chunks=30)
    write_out("FAQ EXIT", text)

# Step 12: Enter portal
time.sleep(1)
tn.read_very_eager()
