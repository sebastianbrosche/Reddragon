...
for i in range(40):
    ...
    if data:
        ...
        # Detect prompt type and respond
        low = text.lower()
        ...
        if "password" in low or "passwd" in low:
            ...
            tn.write(b'y\n')
            time.sleep(0.3)
        elif "press enter" in low or "press return" in low or "press a enter" in low:
            tn.write(b'\n')
            time.sleep(0.3)
        elif "too short" in low or "try again" in low:
            # We're stuck in a name loop, send a valid name
            tn.write(b'archivist\n')
            time.sleep(0.3)
    else:
